...
"""
    ...
    Raises:
        ValueError: If the template is not a string, if the attendees list is empty,
                    or if the attendees list is not a list of dictionaries.
"""
...
def generate_invitations(template, attendees):
    ...
    try:
        ...
        for item in attendees:
            if not isinstance(item, dict):
                raise ValueError("[ERROR] Attendees list must be a list of dictionaries.")
        ...
        print(err)
        return
    #If missing keys in the attendees list and upadate the template them with N/A
    ...
    for item in attendees:
        ...
        invitation = invitation.replace('{{event_location}}', item['event_location'])
